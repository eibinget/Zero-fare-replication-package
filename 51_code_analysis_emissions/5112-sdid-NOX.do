...
/* ************************************************************************* * 
*   Creator: Tobias Eibinger and Sachintactha Fernando
*   Project: Zero Fare
*  Description: Running the analysis at NUTS2level w placebo inference - NOX
*   Date: 2026-04-05
** ************************************************************************* */

*Percapita NOX Emissions - Main Specification
********************************************************************************
clear
use "$final/sdid-dataset-urban"

*manually adjust
reghdfe lnoxcap lgdp acases stringIndex lsome lusual ei ldiesel_real lsuper_real lload lrel_diesel cb_inflow75 intact if ccode!="LU", absorb(id year) vce(cluster id)
...
*gen yadj
gen yadj = lnoxcap - bdiesel*ldiesel_real - bsuper*lsuper_real - bei*ei - bgdp*lgdp - bload*lload - busual*lusual - bcb_inflow75*cb_inflow75 - bintact*intact - bsome*lsome - bidx*stringIndex - bcases*acases - bdrep*lrel_diesel 
...
*check differences in yearly pointact estimates and baseline
...
reghdfe lnoxcap lgdp acases stringIndex lsome lusual ei ldiesel_real lsuper_real lload lrel_diesel cb_inflow75 intact  if id!=`placebo_unit', absorb(id year) vce(cluster id)
...
tw rarea UCI LCI time, color(gray%30) || scatter d time, color(black) m(d) title("NUTS2-Annual") xtitle("") ytitle("lnoxcap") xlab(2016(1)2023, angle(45)) xline(2019, lc(black) lp(solid)) yline(0, lc(black) lp(shortdash)) scheme(sj) legend(off) plotregion(fcolor(white)) graphregion(fcolor(white))

save "$final/es_nox_urban.dta", replace
restore


********************************************************************************
*Percapita Nox Emissions -Robustness check 
*Only dropping NUTS2 regions that are fully rural 
********************************************************************************
clear
use "$final/sdid-dataset-rural.dta", replace
...
save "$final/es_nox_rural", replace
restore
